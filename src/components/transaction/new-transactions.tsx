...
import { getDraftTransactions } from '@/api/transaction'
...
function NewTransactions() {
	...
	const { data, isLoading, refetch } = useFetch<Array<TransactionType>>(
		`draft`,
		getDraftTransactions
	)
	const { moneyInTypes } = useMoneyType()
	const defaultTransaction: TransactionType = useMemo(() => {
		return {
			id: uuid(),
			money: 0,
	...
	const addDraft = (draft?: TransactionType) => {
		const defaultAddedTransaction: TransactionType = {
			id: uuid(),
			money: 0,
			walletName: money[0].name,
			type: 'anuong',
		...
		const newTransaction: TransactionType = draft || defaultAddedTransaction
		setTransactions([...transactions, newTransaction])
	}
	const updateDraft = (id: string, draft: TransactionType) => {
		setTransactions(
